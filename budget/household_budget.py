...
def display_sidebar():
    ...
    if st.session_state.get("logged_in", False):
        st.sidebar.markdown(
            f"Welcome to your *budget tracker*, **{st.session_state.username}**!"
        )
    else:
        st.sidebar.markdown("Please log in or sign up to use the budget tracker.")

    col1, col2 = st.sidebar.columns(2)
    if st.session_state.get("logged_in", False):
        col_sidebar, col_logout = st.sidebar.columns(2)
        # with col_sidebar:
        ...
        with col_logout:
            if st.sidebar.button("🚪 Logout", key="logout_button"):
                ...
                if auth_cookie:
                    delete_session(auth_cookie.value)
                clear_auth_cookie()
...
def get_session(session_id):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT username, expiry FROM sessions WHERE session_id = ?", (session_id,)
    )
    result = c.fetchone()
    conn.close()
...
def set_auth_cookie(session_id):
    ...
    cookie["session_id"]["expires"] = (datetime.now() + timedelta(days=1)).strftime(
        "%a, %d-%b-%Y %H:%M:%S GMT"
    )
    st.query_params["cookie"] = cookie["session_id"].OutputString()


def get_auth_cookie():
    cookie_string = st.query_params.get("cookie", [None])[0]
    if cookie_string:
        cookie = SimpleCookie()
...
def check_session():
    auth_cookie = get_auth_cookie()
    if auth_cookie:
        username = get_session(auth_cookie.value)
        if username:
            ...
            # Session expired or invalid, clear session state
            st.session_state.logged_in = False
            st.session_state.username = None
            st.session_state.currency = "$"
            clear_auth_cookie()
    else:
        # No auth cookie found, check if user is already logged in
...
def add_new_item():
    ...
    with col1:
        ...
        date = st.date_input(
            "Date", value=datetime.now().date(), key="new_item_date_input"
        )
        ...
        if product and not category:
            category = st.selectbox(
                "Category",
                options=list(st.session_state.product_categories.keys()) + ["Other"],
            )

        add_item = st.button("Add Item", key="add_item_button")
...
def load_budget_data():
    if os.path.exists(BUDGET_CSV):
        ...
        if "Date" not in df.columns:
            ...
            ).dt.date  # Convert to date only, flexible format
        return df
...
def save_monthly_budget(monthly_budget):
    pd.DataFrame({"Monthly Budget": [monthly_budget]}).to_csv(
        MONTHLY_BUDGET_CSV, index=False
    )
...
def save_savings_goal(goal):
    pd.DataFrame({"Amount": [goal["amount"]], "Date": [goal["date"]]}).to_csv(
        SAVINGS_GOAL_CSV, index=False
    )
...
def load_spending_restrictions():
    if os.path.exists(SPENDING_RESTRICTIONS_CSV):
        ...
        return {
            row["Product"]: {"limit": row["Limit"], "period": row["Period"]}
            for _, row in df.iterrows()
        }
    return {}
...
def add_recurring_expense():
    ...
    rec_category = st.sidebar.selectbox(
        "Recurring Category", options=list(st.session_state.product_categories.keys())
    )
    rec_frequency = st.sidebar.selectbox(
...
def set_savings_goal():
    ...
    goal_amount = st.sidebar.number_input(
        ...
        step=1.0,
        value=(
            st.session_state.savings_goal["amount"]
            if st.session_state.savings_goal
            else 0.0
        ),
    )
    goal_date = st.sidebar.date_input(
        "Target Date",
        value=(
            st.session_state.savings_goal["date"]
            if st.session_state.savings_goal
...
def add_to_savings():
    ...
    with col1:
        savings_amount = st.number_input(
            ...
            min_value=0.0,
            step=1.00,
            key="savings_amount_input",
        )

    with col2:
        savings_date = st.date_input(
            "Date", value=datetime.now().date(), key="savings_date_input"
        )
    ...
    if add_savings_button:
        if savings_amount > 0:
            ...
            if (
                "savings_goal" in st.session_state
                and st.session_state.savings_goal is not None
            ):
                total_savings = df[df["Product"] == "Savings"]["Amount"].sum()
                goal = st.session_state.savings_goal
                if total_savings >= goal["amount"]:
                    st.success(
                        f"{format_currency(savings_amount)} added to savings! Congratulations! You've reached your savings goal!"
                    )
                else:
                    remaining = goal["amount"] - total_savings
                    st.success(
                        f"{format_currency(savings_amount)} added to savings! {format_currency(remaining)} left to reach your goal."
                    )
            else:
    ...
    if today > st.session_state.last_recurring_check:
        for expense in st.session_state.recurring_expenses:
            if expense["Frequency"] == "Daily":
                ...
                months = (
                    (today.year - st.session_state.last_recurring_check.year) * 12
                    + today.month
                    - st.session_state.last_recurring_check.month
                )
                days_to_add = months
            else:
...
def display_savings_goal_progress():
    ...
    if "savings_goal" in st.session_state and st.session_state.savings_goal is not None:
        ...
        if goal["amount"] > 0:
            ...
            with col1:
                st.write(
                    f"Saved: {format_currency(total_savings)} / {format_currency(goal['amount'])}"
                )
            with col2:
            ...
            if total_savings >= goal["amount"]:
                ...
                st.info(
                    f"To reach your goal, you need to save {format_currency(daily_savings_needed)} per day."
                )
        else:
...
def manage_spending_restrictions():
    ...
    product = st.sidebar.selectbox(
        "Select Product",
        options=[""]
        + list(set([item["Product"] for item in st.session_state.budget_items])),
    )
    if product:
        existing_restriction = st.session_state.spending_restrictions.get(product, {})
        limit = st.sidebar.number_input(
            f"Set limit for {product}",
            min_value=0.0,
            step=1.0,
        ...
        period = st.sidebar.selectbox(
            ...
            index=["Daily", "Weekly", "Monthly"].index(
                existing_restriction.get("period", "Monthly")
            ),
        )
        if st.sidebar.button("Set Restriction"):
            st.session_state.spending_restrictions[product] = {
                "limit": limit,
                "period": period,
            }
            save_spending_restrictions(st.session_state.spending_restrictions)
            st.sidebar.success(
                f"Restriction set for {product}: {format_currency(limit)} {period.lower()}"
            )
...
def display_pie_chart():
    if st.session_state.budget_items:
        ...
        def format_label(category, percentage):
            emoji = (
                category.split()[0] if " " in category else "📊"
        ...
        category_totals["Label"] = category_totals.apply(
            lambda row: format_label(row["Category"], row["Percentage"]), axis=1
        )
        ...
        fig = px.pie(
            ...
            labels="Label",
            hole=0.3,
        )  # Adding a hole to make it a donut chart for better readability

        fig.update_traces(textposition="inside", textinfo="label")
        fig.update_layout(
            showlegend=False
        )  # Hide legend as information is now in the chart
...
def display_spending_chart():
    if st.session_state.budget_items:
        ...
        fig = px.line(
            df, x="Date", y="Cumulative Sum", title="Cumulative Spending Over Time"
        )
        st.plotly_chart(fig)
...
def display_spending_restrictions():
    ...
    if st.session_state.spending_restrictions:
        ...
        restrictions_df["Limit"] = restrictions_df["Limit"].apply(
...
def set_currency():
    ...
    selected_currency = st.sidebar.selectbox(
        "Select Currency", currencies, index=currencies.index(current_currency)
    )

    if selected_currency != current_currency and st.session_state.get(
        "logged_in", False
    ):
        ...
        c.execute(
            ...
            (selected_currency, st.session_state.username),
        )
        conn.commit()
        conn.close()
...
if "savings_goal" not in st.session_state:
    st.session_state.savings_goal = load_savings_goal() or {
        "amount": 0,
...
all_products = [
    item for sublist in st.session_state.product_categories.values() for item in sublist
]
